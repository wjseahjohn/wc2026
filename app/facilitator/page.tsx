...
export default function FacilitatorPage() {
  ...
  function save(updated: FBet[]) {
    localStorage.setItem('fbets', JSON.stringify(updated));
    setFbets(updated);
  }

  function addBet() {
    if (!form.playerName || !form.selection || !form.odds || !form.stake) {
    ...
    const bet: FBet = {
      ...
      matchLabel: match
        ...
        : (form.betType==='winner'?'Tournament Winner':form.betType==='scorer'?'Top Scorer':'—'),
  ...
  function toggle(id: string) {
    save(fbets.map(b => b.id===id ? {...b, placed:!b.placed} : b));
  }

  function del(id: string) {
    if (confirm('Delete this bet?')) save(fbets.filter(b=>b.id!==id));
  }
  ...
  if (!authed) return (
    <div style={{minHeight:'100vh',background:'#071f10',display:'flex',alignItems:'center',justifyContent:'center',padding:'16px',fontFamily:'system-ui,sans-serif'}}>
      <div style={{background:'rgba(255,255,255,0.05)',border:'1px solid rgba(245,200,66,0.3)',borderRadius:'16px',padding:'40px 24px',maxWidth:'360px',width:'100%',textAlign:'center'}}>
        ...
        <div style={{fontSize:'12px',color:'#a0a09a',marginBottom:'20px'}}>Record and track bets for SGPools</div>
        <input type="password" placeholder="Admin key..." value={key}
          onChange={e=>setKey(e.target.value)}
          onKeyDown={e=>{if(e.key==='Enter'&&key)setAuthed(true);}}
          style={{width:'100%',padding:'12px',borderRadius:'10px',border:'none',fontSize:'16px',textAlign:'center',background:'#f0ede4',color:'#071f10',marginBottom:'12px',outline:'none',fontFamily:'inherit'}}
          autoFocus />
  ...
  return (
    <div style={{minHeight:'100vh',background:'#071f10',color:'#f0ede4',paddingBottom:'40px',fontFamily:'system-ui,sans-serif'}}>
        ...
        <div style={{maxWidth:'700px',margin:'0 auto'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'10px'}}>
            <div>
              ...
              <div style={{fontSize:'11px',color:'#a0a09a'}}>Track and place on SGPools</div>
            </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:'6px',marginBottom:'10px'}}>
            {[
              ...
              <div key={stat.l} style={{background:'rgba(255,255,255,0.05)',borderRadius:'8px',padding:'7px',textAlign:'center'}}>
                <div style={{fontWeight:900,fontSize:'15px',color:stat.c}}>{stat.v}</div>
                <div style={{fontSize:'10px',color:'#a0a09a'}}>{stat.l}</div>
              </div>
            ))}
          </div>
      ...
      <div style={{maxWidth:'700px',margin:'0 auto',padding:'16px'}}>
        {msg && (
          <div style={{padding:'10px',borderRadius:'10px',marginBottom:'12px',fontWeight:600,fontSize:'13px',background:msg.startsWith('✅')?'rgba(74,222,128,0.15)':'rgba(248,113,113,0.15)',color:msg.startsWith('✅')?'#4ade80':'#f87171'}}>
            {msg}
          </div>
        )}
        ...
        {view === 'add' && (
          <div style={{display:'flex',flexDirection:'column',gap:'14px'}}>
            ...
            <div>
              <div style={{fontSize:'11px',color:'#a0a09a',marginBottom:'5px',letterSpacing:'1px',fontWeight:600}}>FAMILY MEMBER</div>
              <input value={form.playerName} onChange={e=>setForm(f=>({...f,playerName:e.target.value}))}
                placeholder="e.g. Dad, Mum, John..."
            ...
            <div>
              <div style={{fontSize:'11px',color:'#a0a09a',marginBottom:'5px',letterSpacing:'1px',fontWeight:600}}>SGPOOLS BET TYPE</div>
            ...
            {!['winner','scorer'].includes(form.betType) && (
              <div>
                <div style={{fontSize:'11px',color:'#a0a09a',marginBottom:'5px',letterSpacing:'1px',fontWeight:600}}>MATCH</div>
                ...
                {selectedMatch && (
                  <div style={{marginTop:'8px',background:'rgba(255,255,255,0.05)',borderRadius:'10px',padding:'12px'}}>
                    <div style={{fontSize:'11px',color:'#a0a09a',fontWeight:700,marginBottom:'8px'}}>ODDS REFERENCE</div>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'6px',marginBottom:'6px',textAlign:'center'}}>
                      {[[selectedMatch.homeTeam,selectedMatch.odds1x2.home],['Draw',selectedMatch.odds1x2.draw],[selectedMatch.awayTeam,selectedMatch.odds1x2.away]].map(([l,o]:any,i:number)=>(
                        <div key={i} style={{background:'rgba(255,255,255,0.05)',borderRadius:'8px',padding:'7px'}}>
                          <div style={{fontSize:'10px',color:'#a0a09a',marginBottom:'2px'}}>{l}</div>
                          <div style={{fontWeight:900,fontSize:'16px',color:'#f5c842'}}>{Number(o).toFixed(2)}</div>
                        </div>
                      ))}
                    </div>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:'5px',textAlign:'center'}}>
                      {[['O2.5',selectedMatch.oddsOU.over],['U2.5',selectedMatch.oddsOU.under],['BTTS Y',selectedMatch.oddsBTTS.yes],['BTTS N',selectedMatch.oddsBTTS.no]].map(([l,o]:any,i:number)=>(
                        <div key={i} style={{background:'rgba(255,255,255,0.05)',borderRadius:'8px',padding:'6px'}}>
                          <div style={{fontSize:'10px',color:'#a0a09a',marginBottom:'2px'}}>{l}</div>
                          <div style={{fontWeight:700,fontSize:'13px'}}>{Number(o).toFixed(2)}</div>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div>
              <div style={{fontSize:'11px',color:'#a0a09a',marginBottom:'5px',letterSpacing:'1px',fontWeight:600}}>SELECTION</div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
              <div>
                <div style={{fontSize:'11px',color:'#a0a09a',marginBottom:'5px',letterSpacing:'1px',fontWeight:600}}>ODDS (from SGPools)</div>
                <input type="number" step="0.01" value={form.odds} onChange={e=>setForm(f=>({...f,odds:e.target.value}))}
                  placeholder="e.g. 1.85"
                  style={{width:'100%',padding:'12px',borderRadius:'10px',border:'none',background:'#f0ede4',color:'#071f10',fontWeight:600,fontSize:'15px',outline:'none',fontFamily:'inherit'}} />
              </div>
              <div>
                <div style={{fontSize:'11px',color:'#a0a09a',marginBottom:'5px',letterSpacing:'1px',fontWeight:600}}>STAKE (SGD $)</div>
                <input type="number" step="1" value={form.stake} onChange={e=>setForm(f=>({...f,stake:e.target.value}))}
                  placeholder="e.g. 10"
                  style={{width:'100%',padding:'12px',borderRadius:'10px',border:'none',background:'#f0ede4',color:'#071f10',fontWeight:600,fontSize:'15px',outline:'none',fontFamily:'inherit'}} />
              </div>
            </div>
            ...
            <button onClick={addBet}
              style={{padding:'14px',borderRadius:'12px',border:'none',cursor:'pointer',fontWeight:900,fontSize:'16px',letterSpacing:'1px',background:'#f5c842',color:'#071f10'}}>
              ➕ ADD BET
            </button>
          </div>
        )}
        ...
        {view === 'byMatch' && (
          <div>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'14px'}}>
              ...
              {unplaced.length > 0 && (
                <span style={{fontSize:'11px',padding:'4px 8px',borderRadius:'20px',background:'rgba(232,144,26,0.2)',color:'#e8901a',fontWeight:700}}>
                ...
                {Object.entries(matchGroups).map(([matchLabel, matchBets]) => {
                  ...
                  return (
                    <div key={matchLabel} style={{background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'12px',overflow:'hidden'}}>
                      <div style={{padding:'10px 14px',background:'rgba(245,200,66,0.08)',borderBottom:'1px solid rgba(255,255,255,0.08)',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:'6px'}}>
                        ...
                        <div key={b.id} style={{padding:'10px 14px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:'8px',borderBottom:'1px solid rgba(255,255,255,0.05)',background:b.placed?'rgba(74,222,128,0.04)':'transparent'}}>
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{display:'flex',gap:'5px',alignItems:'center',marginBottom:'2px',flexWrap:'wrap'}}>
                              ...
                              <span style={{fontSize:'10px',padding:'1px 5px',borderRadius:'6px',background:'rgba(59,130,246,0.2)',color:'#60a5fa'}}>
                                {BET_TYPES.find(x=>x.v===b.betType)?.l||b.betType}
                              </span>
                            </div>
                          ...
                          <div style={{display:'flex',flexDirection:'column',gap:'4px',alignItems:'flex-end',flexShrink:0}}>
                            ...
                            <button onClick={()=>toggle(b.id)}
                              style={{padding:'4px 8px',borderRadius:'6px',border:`1px solid ${b.placed?'rgba(74,222,128,0.4)':'rgba(232,144,26,0.4)'}`,background:b.placed?'rgba(74,222,128,0.15)':'transparent',color:b.placed?'#4ade80':'#e8901a',cursor:'pointer',fontWeight:700,fontSize:'10px',whiteSpace:'nowrap'}}>
