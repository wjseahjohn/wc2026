...
export async function POST(req: NextRequest) {
  ...
  if (resetMatchId) {
    ...
    const updated = bets.map((b:any) => {
      ...
      const betMatchId = b.targetId.includes('_') ? ((b.targetId.startsWith('R32')||b.targetId.startsWith('R16')) ? b.targetId.split('_').slice(0,2).join('_') : b.targetId.split('_')[0]) : b.targetId;
