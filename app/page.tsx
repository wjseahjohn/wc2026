...
function formatSelection(b: any, matches: any[]): string {
  ...
  const bt = b.betType;
  const mid = ((b.targetId?.startsWith('R32')||b.targetId?.startsWith('R16')||b.targetId?.startsWith('QF')||b.targetId?.startsWith('SF')) ? b.targetId?.split('_').slice(0,2).join('_') : b.targetId?.split('_')[0]);
                        ...
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'8px'}}>
                          <div style={{flex:1,minWidth:0}}>
                            ...
                            <div style={{fontSize:'12px',color:'#f0ede4',marginBottom:'2px'}}>
                              {(()=>{const mid=((b.targetId?.startsWith('R32')||b.targetId?.startsWith('R16')||b.targetId?.startsWith('QF')||b.targetId?.startsWith('SF')) ? b.targetId?.split('_').slice(0,2).join('_') : b.targetId?.split('_')[0]);const m=(data?.matches||[]).find((x:any)=>x.id===mid);return m?m.homeTeam+' vs '+m.awayTeam:'';})()}
                            </div>
