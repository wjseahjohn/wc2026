...
function formatSelection(b: any, matches: any[]): string {
  ...
  const bt = b.betType;
  const mid = ((b.targetId?.startsWith('R32')||b.targetId?.startsWith('R16')) ? b.targetId?.split('_').slice(0,2).join('_') : b.targetId?.split('_')[0]);
        ...
        {tab === 'matches' && (
          <div>
            <div style={{marginBottom:'14px'}}>
              <div style={{fontSize:'22px',fontWeight:900,color:'#f5c842',letterSpacing:'2px'}}>{matchStatus==='completed'?'COMPLETED MATCHES':group==='R16'?'ROUND OF 16':group==='R32'?'ROUND OF 32':'GROUP STAGE'}</div>
            </div>
                        ...
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'8px'}}>
                          <div style={{flex:1,minWidth:0}}>
                            ...
                            <div style={{fontSize:'12px',color:'#f0ede4',marginBottom:'2px'}}>
                              {(()=>{const mid=((b.targetId?.startsWith('R32')||b.targetId?.startsWith('R16')) ? b.targetId?.split('_').slice(0,2).join('_') : b.targetId?.split('_')[0]);const m=(data?.matches||[]).find((x:any)=>x.id===mid);return m?m.homeTeam+' vs '+m.awayTeam:'';})()}
                            </div>
