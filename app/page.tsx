...
function formatSelection(b: any, matches: any[]): string {
  ...
  const bt = b.betType;
  const mid = ((b.targetId?.startsWith('R32')||b.targetId?.startsWith('R16')||b.targetId?.startsWith('QF')) ? b.targetId?.split('_').slice(0,2).join('_') : b.targetId?.split('_')[0]);
        ...
        {tab === 'matches' && (
          <div>
            <div style={{marginBottom:'14px'}}>
              <div style={{fontSize:'22px',fontWeight:900,color:'#f5c842',letterSpacing:'2px'}}>{matchStatus==='completed'?'COMPLETED MATCHES':group==='QF'?'QUARTER FINALS':group==='R16'?'ROUND OF 16':group==='R32'?'ROUND OF 32':'GROUP STAGE'}</div>
            </div>
          ...
          <div>
            ...
            {!namedIn ? (
              ...
              <>
                ...
                {/* Balance card */}
                <div style={{background:'rgba(255,255,255,0.05)',border:'1px solid '+(myBalance>0?'rgba(248,113,113,0.3)':myBalance<0?'rgba(74,222,128,0.3)':'rgba(255,255,255,0.15)'),borderRadius:'14px',padding:'16px',marginBottom:'16px'}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'8px',marginBottom:'10px'}}>
                    {[
                      ...
                      <div key={x.l} style={{textAlign:'center',padding:'10px',borderRadius:'10px',background:'rgba(255,255,255,0.04)'}}>
                        <div style={{fontWeight:900,fontSize:'15px',color:x.c}}>{x.v}</div>
                        <div style={{fontSize:'10px',color:'#a0a09a',marginTop:'2px'}}>{x.l}</div>
                      </div>
                    ))}
                  </div>
                        ...
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'8px'}}>
                          <div style={{flex:1,minWidth:0}}>
                            ...
                            <div style={{fontSize:'12px',color:'#f0ede4',marginBottom:'2px'}}>
                              {(()=>{const mid=((b.targetId?.startsWith('R32')||b.targetId?.startsWith('R16')||b.targetId?.startsWith('QF')) ? b.targetId?.split('_').slice(0,2).join('_') : b.targetId?.split('_')[0]);const m=(data?.matches||[]).find((x:any)=>x.id===mid);return m?m.homeTeam+' vs '+m.awayTeam:'';})()}
                            </div>
