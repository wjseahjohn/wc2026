...
function didBetWin(bet: Bet, result: MatchResult): boolean {
  ...
  const matchId = (bet.targetId.startsWith('R32')||bet.targetId.startsWith('R16')) ? bet.targetId.split('_').slice(0,2).join('_') : bet.targetId.split('_')[0];
...
export async function setMatchResult(matchId: string, result: MatchResult): Promise<void> {
  ...
  const updated = all.map(bet => {
    const betMatchId = (bet.targetId.startsWith('R32')||bet.targetId.startsWith('R16')) ? bet.targetId.split('_').slice(0,2).join('_') : bet.targetId.split('_')[0];
